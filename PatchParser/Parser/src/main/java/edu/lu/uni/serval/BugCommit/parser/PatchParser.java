...
public class PatchParser {
	...
	public void getBugPosition(List<HierarchicalActionSet> actionSets, File prevFile, File revFile) {
		CUCreator cuCreator = new CUCreator();
		CompilationUnit prevUnit = cuCreator.createCompilationUnit(prevFile);
		CompilationUnit revUnit = cuCreator.createCompilationUnit(revFile);
		for (HierarchicalActionSet actionSet : actionSets) {
			//System.out.println("bugstartnum:" + setLineNumbers(actionSet, prevUnit, revUnit));
			setLineNumbers(actionSet, prevUnit, revUnit);
	...
	public List<HierarchicalActionSet> parseChangedSourceCodeWithGumTreeSub(File prevFile, File revFile) {
		List<HierarchicalActionSet> actionSets = new ArrayList<>();
		// GumTree results
		List<Action> gumTreeResults = new GumTreeComparer().compareTwoFilesWithGumTree(prevFile, revFile);
		if (gumTreeResults == null) {
